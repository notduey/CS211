...
// YOUR CODE GOES HERE
    ...
    private int size(IntTreeNode current) { // helper method, does actual recursive work
        if (current == null) { // base case
            return 0;
        }

        // current node + nodes in left subtree + nodes in right subtree
        return 1 + size(current.left) + size(current.right); // recursive case
    }

    // b. return number of empty branches (null links/childrens)
    ...
    private int countEmpty(IntTreeNode current) {
        if (current == null) { // base case
            return 1; // empty subtree counts as 1 empty branch
        }

        // empty branches in left subtree + empty branches in right subtree
        return countEmpty(current.left) + countEmpty(current.right); // recursive case
        // recursively goes down left and right subtree and counts null links
    }

    // c. return sum of value stores in tree, values * weight (depth of node)
    public int depthSum() {
        return depthSum(overallRoot, 1); // root has weight 1
    }

    private int depthSum(IntTreeNode current, int weight) {
        if (current == null) { // base case
            return 0; // empty subtree counts as 0
        }

        // current node sum (weight * data) + sum of left subtree + sum of right subtree
        return weight * current.data
                + depthSum(current.left, weight + 1) // increment weight as we go down 
                + depthSum(current.right, weight + 1);
    }

    // d. outputs leaves in reverse order (right to left)
    public void printLeaves() {
        if (overallRoot == null) { // if tree is empty
    ...
    private void printLeaves(IntTreeNode current) {
        ...
        // if current node has no children (leaf), print it
        if (current.left == null && current.right == null) {
            System.out.print(" " + current.data);
        }
        else { // else keep recursing down
            // right then left (reverse order)
