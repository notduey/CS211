...
// YOUR CODE GOES HERE
    ...
    public int longestSortedSequence() {
        ...
        for (int i = 0; i < this.size -1; i++) {
            if (this.get(i) <= this.get(i + 1)) { // <= for duplicates
                count++;
            }
            else {
                if (count > longest) {
    ...
    public void removeFront(int n) {
        // shift remaining elements to the left by n
        for (int i = 0; i < n; i++) {
    ...
    // example [8, 17, 9, 24, 42, 3, 8]
    // removeFront(4)

    //elementData[0] = elementData[4] -> 42
    //elementData[1] = elementData[5] -> 3
    //elementData[2] = elementData[6] -> 8
    // result:[42, 3, 8, 24, 42, 3, 8]
    ...
    // 4.
    public void mirror() {
        ensureCapacity(size * 2); // checks if array is big enough to hold 2x size
    ...
    public ArrayIntList fromCounts() {
        ...
        for (int i = 0; i < this.size; i += 2) {
            ...
            for (int j = 0; j < this.get(i); j++) {
                newList.add(this.get(i + 1));
            }
        }
